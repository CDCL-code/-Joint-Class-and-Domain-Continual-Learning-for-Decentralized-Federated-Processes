...
class NCNDScenario(GenericCLScenario['NCExperience']):
    """
    ...
    def __init__(self, train_dataset: AvalancheDataset,
                 ...
                 train_dataset2: AvalancheDataset,
                 test_dataset2: AvalancheDataset,
                 n_experiences: int,
                 task_labels: bool,
                 shuffle: bool = True,
        ...
        if reproducibility_data:
            ...
            self.n_classes_per_exp = [self.n_classes // int(n_experiences/2)] * int(n_experiences/2)
            print(n_experiences)
            print(self.n_classes_per_exp)
        ...
        if reproducibility_data:
            ...
            print(self.classes_order_original_ids)
        ...
        for exp_id in range(int(n_experiences/2)):
        ...
        for t_id, exp_def in enumerate(train_exps_patterns_assignment):
            ...
            task_labels = ConstantSequence(train_task_labels[-1],
                                           len(train_dataset))
            subset=AvalancheSubset(train_dataset, indices=exp_def,
                                task_labels=task_labels)
            
            curr_t=set(subset.targets)
            print(curr_t)
            train_experiences.append(subset
                )
        ...
        for t_id, exp_def in enumerate(test_exps_patterns_assignment):
            ...
            test_experiences.append(AvalancheSubset(test_dataset, indices=exp_def,
                                task_labels=task_labels))
        ...
        self.classes_order_original_ids: List[int] = torch.unique(
            torch.as_tensor(train_dataset2.targets),
            sorted=True).tolist()
        ...
        original_training_dataset = train_dataset2
        original_test_dataset2 = test_dataset2
        ...
        for exp_id in range(int(n_experiences/2)):
            ...
            selected_indexes_test = []
            for idx, element in enumerate(original_test_dataset2.targets):
                if element in selected_classes:
                    selected_indexes_test.append(idx)
                    if self._has_task_labels:
                        pattern_test_task_labels[idx] = exp_id
        ...
        # Good idea, but doesn't work
        # transform_groups = train_eval_transforms(train_dataset2, test_dataset2)
        #
        ...
        train_dataset2 = AvalancheSubset(
            train_dataset2, class_mapping=self.class_mapping,
            initial_transform_group='train')
        test_dataset2 = AvalancheSubset(
            test_dataset2, class_mapping=self.class_mapping,
            initial_transform_group='eval')

        self.train_exps_patterns_assignment = train_exps_patterns_assignment
        """ A list containing which training instances are assigned to each
        experience in the train stream. Instances are identified by their id 
        w.r.t. the dataset found in the original_train_dataset2 field. """

        self.test_exps_patterns_assignment = test_exps_patterns_assignment
        """ A list containing which test instances are assigned to each
        experience in the test stream. Instances are identified by their id 
        w.r.t. the dataset found in the original_test_dataset2 field. """
        ...
        for t_id, exp_def in enumerate(train_exps_patterns_assignment):
            ...
            task_labels = ConstantSequence(train_task_labels[-1],
                                           len(train_dataset2))
            subset=AvalancheSubset(train_dataset2, indices=exp_def,
                                task_labels=task_labels)
        ...
        for t_id, exp_def in enumerate(test_exps_patterns_assignment):
            ...
            test_experiences.append(AvalancheSubset(test_dataset2, indices=exp_def,
                                task_labels=task_labels))
        if seed!=None:
            ...
            print('order: ')
            print(order)
            train_experiences=[train_experiences[i] for i in order]
            test_experiences=[test_experiences[i] for i in order]
            
        super(NCNDScenario, self).__init__(
            stream_definitions={
...
class NCNDScenariox2(GenericCLScenario['NCExperience']):
    ...
    def __init__(self, train_dataset: AvalancheDataset,
        ...
        original_training_dataset2 = train_dataset2
        original_test_dataset2 = test_dataset2
        ...
        for exp_id in range(int(n_experiences/2)):
            selected_classes = self.original_classes_in_exp[exp_id]
            selected_indexes_train = []
            for idx, element in enumerate(original_training_dataset2.targets):
                if element in selected_classes:
                    selected_indexes_train.append(idx)
                    if self._has_task_labels:
                        pattern_train_task_labels[idx] = exp_id
        ...
        if seed:
            ...
            random.seed(seed)
            random.shuffle(test_experiences)
        super(NCNDScenariox2, self).__init__(
            stream_definitions={
                'train': (train_experiences, train_task_labels, train_dataset),
                'test': (test_experiences, test_task_labels, test_dataset)
            },
...
class NCNDScenario3(GenericCLScenario['NCExperience']):
    """
    This class defines a "New Classes" scenario. Once created, an instance
    of this class can be iterated in order to obtain the experience sequence
    under the form of instances of :class:`NCExperience`.
    ...
    def __init__(self, train_dataset: AvalancheDataset,
                 ...
                 train_dataset2: AvalancheDataset,
                 test_dataset2: AvalancheDataset,
                 train_dataset3: AvalancheDataset,
                 test_dataset3: AvalancheDataset,
                 n_experiences: int,
                 task_labels: bool,
                 shuffle: bool = True,
                 seed: Optional[int] = None,
        ...
        if reproducibility_data:
            ...
            '''if self.n_classes % n_experiences > 0:
                raise ValueError(
                    ...
                    f'n_experiences ({n_experiences})')'''
            self.n_classes_per_exp = \
                [self.n_classes // int(n_experiences/2)] * int(n_experiences/2)
        ...
        self.classes_order_original_ids: List[int] = torch.unique(
            torch.as_tensor(train_dataset3.targets),
            sorted=True).tolist()
        ...
        original_training_dataset = train_dataset3
        original_test_dataset3 = test_dataset3
        ...
        for exp_id in range(int(n_experiences/2)):
            ...
            selected_indexes_test = []
            for idx, element in enumerate(original_test_dataset3.targets):
                if element in selected_classes:
                    selected_indexes_test.append(idx)
                    if self._has_task_labels:
                        pattern_test_task_labels[idx] = exp_id
        ...
        train_dataset3 = AvalancheSubset(
            train_dataset3, class_mapping=self.class_mapping,
            initial_transform_group='train')
        test_dataset3 = AvalancheSubset(
            test_dataset3, class_mapping=self.class_mapping,
            initial_transform_group='eval')
        ...
        for t_id, exp_def in enumerate(train_exps_patterns_assignment):
            ...
            task_labels = ConstantSequence(train_task_labels[-1],
                                           len(train_dataset3))
            subset=AvalancheSubset(train_dataset3, indices=exp_def,
                                task_labels=task_labels)
        ...
        for t_id, exp_def in enumerate(test_exps_patterns_assignment):
            ...
            test_experiences.append(AvalancheSubset(test_dataset3, indices=exp_def,
                                task_labels=task_labels))
        
        super(NCNDScenario3, self).__init__(
            stream_definitions={
                'train': (train_experiences, train_task_labels, train_dataset),
                'test': (test_experiences, test_task_labels, test_dataset)
            },
