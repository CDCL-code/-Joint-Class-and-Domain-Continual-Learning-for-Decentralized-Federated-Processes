...
from avalanche.benchmarks.scenarios.new_classes.nc_scenario import \
    NCScenario,NCNDScenario,NCNDScenario3, NCNDScenariox2
...
def nc_nd_benchmark(
    ...
    if isinstance(train_dataset, list) or isinstance(train_dataset, tuple):
        ...
        '''seq_train_dataset=torch.utils.data.ConcatDataset(train_dataset)
        seq_test_dataset=torch.utils.data.ConcatDataset(test_dataset)
        mapping=[]'''
        #print(mapping)
    ...
    if isinstance(train_dataset2, list) or isinstance(train_dataset2, tuple):
        # Multi-dataset setting

        if len(train_dataset2) != len(test_dataset2):
            raise ValueError('Train/test dataset lists must contain the '
                             'exact same number of datasets')
        ...
        seq_train_dataset2, seq_test_dataset2, mapping = \
            concat_datasets_sequentially(train_dataset2, test_dataset2)
        '''seq_train_dataset2=torch.utils.data.ConcatDataset(train_dataset2)
        seq_test_dataset2=torch.utils.data.ConcatDataset(test_dataset2)
        mapping=[]'''
        #print(mapping)
        ...
        if one_dataset_per_exp:
            ...
            # Overrides n_experiences (and per_experience_classes, already done)
            n_experiences = len(train_dataset2)
        train_dataset2, test_dataset2 = seq_train_dataset2, seq_test_dataset2
    ...
    return NCNDScenario(train_dataset, test_dataset,train_dataset2, test_dataset2, n_experiences, task_labels,
...
def nc_nd_benchmarkx2(
    ...
    return NCNDScenariox2(train_dataset, test_dataset,train_dataset2, test_dataset2, n_experiences, task_labels,
                      shuffle, seed, fixed_class_order, per_exp_classes,
                      class_ids_from_zero_from_first_exp,
                      class_ids_from_zero_in_each_exp,
                      reproducibility_data)
...
def nc_nd_benchmark3(
    ...
    if isinstance(train_dataset3, list) or isinstance(train_dataset3, tuple):
        # Multi-dataset setting

        if len(train_dataset3S) != len(test_dataset3):
            raise ValueError('Train/test dataset lists must contain the '
                             'exact same number of datasets')
        ...
        seq_train_dataset3, seq_test_dataset3, mapping = \
            concat_datasets_sequentially(train_dataset3, test_dataset3)
        '''seq_train_dataset2=torch.utils.data.ConcatDataset(train_dataset2)
        seq_test_dataset2=torch.utils.data.ConcatDataset(test_dataset2)
        mapping=[]'''
        #print(mapping)
        ...
        if one_dataset_per_exp:
            ...
            # Overrides n_experiences (and per_experience_classes, already done)
            n_experiences = len(train_dataset3)
        train_dataset3, test_dataset3 = seq_train_dataset3, seq_test_dataset3
    ...
    return NCNDScenario3(train_dataset, test_dataset,train_dataset2, test_dataset2,train_dataset3, test_dataset3, n_experiences, task_labels,
                      shuffle, seed, fixed_class_order, per_exp_classes,
                      class_ids_from_zero_from_first_exp,
                      class_ids_from_zero_in_each_exp,
                      reproducibility_data)
