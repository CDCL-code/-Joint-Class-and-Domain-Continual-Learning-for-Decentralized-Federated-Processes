...
class BaseStrategy:
    ...
    **Evaluation loop**
    ...
    def __init__(self, model: Module, optimizer: Optimizer,
        ...
        '''list of epoch saved for early stopping check'''
        self.early_stopping_epoch=list()
        self.clock = Clock()
    ...
    def train_exp(self, experience: Experience, eval_streams=None, **kwargs):
        ...
        if self.eval_every > 0 and \
            ...
            do_final = False
        '''if self.multi:
            epoch_multi=self.experience.current_experience+1
        else:'''
        
        '''self.fixed_flag=False
        print(self.experience.current_experience)
        if self.experience.current_experience>0:
                print('here fixed')
                self.fixed_flag=True'''
            
        for epoch in range(self.train_epochs):
            self.experience.epoch=epoch
            print(epoch)
            ...
            '''if self.fixed_flag:
                    if epoch < 2:
                        ...
                        print('now f')
                        for lay, (name,param) in enumerate(self.model.named_parameters()):
                            print(lay)
                            if lay==9:
        ...
        #self.early_stopping_epoch.append(self.plugins[0].epoch_to_use)
    ...
    def make_eval_dataloader(self, num_workers=0, pin_memory=True,
        ...
        curr_t=set(self.adapted_dataset.targets)
        print(curr_t)
    ...
    def training_epoch(self, **kwargs):
        ...
        for i,self.mbatch in enumerate(self.dataloader):
            #print(self._stop_training)
            #print(self.num_iter_per_exp)
            #print(i)
            if self.num_iter_per_exp != None:
                if self._stop_training or i>self.num_iter_per_exp:
                    break
            else:
